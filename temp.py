...
crt_id = '''\n<criterion test_ref="{data[test_id]}" comment="Check for {data[Comment]}"/>'''
...
tst_jun = '''\n<show_test xmlns="http://oval.mitre.org/XMLSchema/oval-definitions-5#{data[product]}" id="{data[test_id]}" check_existence="at_least_one_exists" comment="Check for {data[Comment]}" version="1" check="all" >
      <object object_ref="oval:org.tanium.{data[product]}.cve:obj:{data[object_id]}"/>
      <state state_ref="{data[state_id]}"/>
</show_test>'''
